...
export const validatePassword = (password: string): { isValid: boolean; message: string } => {
  if (password.length < 10) {
    return { isValid: false, message: 'A senha deve ter pelo menos 10 caracteres' };
  }
  if (!/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+\-=[\]{};':"\\|,.<>/?])/.test(password)) {
    return { isValid: false, message: 'A senha deve conter letra maiúscula, minúscula, número e caractere especial (!@#$%...)' };
  }
