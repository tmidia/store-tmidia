...
const ProductList = ({ produtos, onAdicionarAoCarrinho, caixaAberto, modoConsulta }: ProductListProps) => {
  ...
  console.log('📦 ProductList renderizando:', { 
    quantidadeProdutos: produtos.length, 
    caixaAberto, 
    modoConsulta,
    produtos: produtos.slice(0, 2) // Log dos primeiros produtos para debug
  });
    ...
    <Card className={`border-0 shadow-md bg-white h-full ${classicCard}`}>
      ...
      <CardContent className="p-0">
        <div className="max-h-96 overflow-y-auto">
          {produtos.length === 0 ? (
            <p className="text-gray-500 text-center py-10 group-[.pdv-classic]:text-slate-700">
              ...
              {produtos.map(produto => (
                <div key={produto.id} className="flex items-center justify-between p-3 border rounded group-[.pdv-classic]:border-slate-400 group-[.pdv-classic]:bg-slate-200">
                  <div className="flex-1">
                    ...
                    <p className="text-lg font-bold text-green-600 group-[.pdv-classic]:text-green-800">
                      R$ {produto.sale_price?.toFixed(2) || '0.00'}
                    </p>
                    <p className="text-xs text-gray-500 group-[.pdv-classic]:text-slate-600">
